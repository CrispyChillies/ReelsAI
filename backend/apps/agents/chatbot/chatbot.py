...
class Chatbot:
    ...
    def _build_workflow(self) -> CompiledStateGraph:
        ...
        workflow.add_conditional_edges(
            "agent", tools_condition, {"tools": "tools", "__end__": "save_to_database"}
        )
        ...
        try:
            ...
            if not created:
                ...
                user_message_count = sum(
                    1 for msg in state["messages"] if msg.type == "human"
                )
    ...
    def process_message(self, request: ChatRequest) -> ChatResponse:
        ...
        try:
            # Validate input
            is_valid, error_message = MessageValidator.validate_user_message(
            ...
            logger.info(
                f"Processed message for user {request.user_id}: tool_used={tool_used}"
            )
            return ChatResponse(
                success=True,
            ...
            error_session_id = (
                session_id or f"session_{request.user_id}_{int(time.time())}"
            )
            return ChatResponse(
                success=False,
